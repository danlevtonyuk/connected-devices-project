...
class Recognizer(AudioSource):
    ...
    def recognize_sphinx(self, audio_data, language="en-us", keyword_entries=None, grammar=None, show_all=False):
        ...
        my_project_directory = "/home/pi/.local/lib/python3.5/site-packages/pocketsphinx/model/en-us"
        ...
        if isinstance(language, str):  # directory containing language data
            ...
            #language_directory = os.path.join(my_project_directory)
            ...
            pass # acoustic_parameters_directory, language_model_file, phoneme_dictionary_file = language
        ...
        config.set_string("-hmm",  os.path.join(self.model_path, language))
        #print("-hmm path:", os.path.join(self.model_path, language))
        config.set_string("-lm",   "/home/pi/final_project/connected-devices-project/recordings/TAR6461/TAR6461/6461.lm")
        config.set_string("-dict", "/home/pi/final_project/connected-devices-project/recordings/TAR6461/TAR6461/6461.dic")

        config.set_string("-logfn", os.devnull)
        #config.set_string("-logfn", "/home/pi/final_project/connected-devices-project/recordings/pocketsphinx_log.txt")  # disable logging (logging causes unwanted output in terminal)
        ...
        if keyword_entries is not None:  # explicitly specified set of keywords
            ...
            if not os.path.exists(fsg_path):  # create FSG grammar if not available
                jsgf = Jsgf(grammar)
                rule = jsgf.get_rule("my_lampi.action") # ("{0}.{0}").format(grammar_name))
                fsg = jsgf.build_fsg(rule, decoder.get_logmath(), 7.5)
                fsg.writefile(fsg_path)
                print("fsg_path:", fsg_path)
            else:
                fsg = FsgModel(fsg_path, decoder.get_logmath(), 7.5)
            decoder.set_fsg(fsg_path, fsg)
            #print("fsg", fsg)
